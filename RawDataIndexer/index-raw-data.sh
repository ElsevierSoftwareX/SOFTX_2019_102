...
./bin/RDI OPTIMIZED_FASTBIT:INDEX ofb-idx-f1 $CPATH/ofb-idx-f1 file-1.csv [customerID:numeric,country:text,continent:text] -delimiter="," -bin="$FASTBIT/bin"
./bin/RDI FASTBIT:INDEX fb-idx-f1 $CPATH/fb-idx-f1 file-1.csv [customerID:numeric,country:text,continent:text] -delimiter="," -bin="$FASTBIT/bin"
...
./bin/RDI OPTIMIZED_FASTBIT:INDEX ofb-idx-f2 $CPATH/ofb-idx-f2 file-2.csv [customerID:numeric,age:numeric,gender:text,children:numeric,status:text] -delimiter="," -bin="$FASTBIT/bin"
./bin/RDI FASTBIT:INDEX fb-idx-f2 $CPATH/fb-idx-f2 file-2.csv [customerID:numeric,age:numeric,gender:text,children:numeric,status:text] -delimiter="," -bin="$FASTBIT/bin"
